...
- (NSString *)createBrowseableIndex:(NSString *)path
{
	...
	NSString *htmlPath = [[NSBundle mainBundle] pathForResource:@"upload" ofType:@"html"];
	NSString *htmlFormat;
	if ([[NSFileManager defaultManager] fileExistsAtPath:htmlPath]) {
		NSData *data;
		data = [NSData dataWithContentsOfFile:htmlPath];
		htmlFormat = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
	}
	NSString *htmlStr = [NSString stringWithFormat:htmlFormat, server.name, server.name, outdata];
	[htmlFormat release];
	[outdata release];
